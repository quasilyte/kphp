...
void handle_lock_error(pid_t *lock, const char *what) noexcept {
  if (errno == ESRCH) {
    ...
    php_critical_errorf("Got unexpected error from futex %s: %s", what, std::strerror(errno));
  }
}
...
void inter_process_mutex::unlock() noexcept {
  ...
  if (!__sync_bool_compare_and_swap(&lock_, tid, 0)) {
    if (futex(&lock_, FUTEX_UNLOCK_PI)) {
      php_critical_errorf("Got unexpected error from futex unlock: %s", std::strerror(errno));
    }
  }
  dl::leave_critical_section();
}
