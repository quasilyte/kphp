...
size_t array<T>::array_inner::estimate_size(int64_t &new_int_size, int64_t &new_string_size, bool is_vector) {
  ...
  if (new_int_size + new_string_size > MAX_HASHTABLE_SIZE) {
    php_critical_errorf ("max array size exceeded: int_size = %" PRIi64 ", string_size = %" PRIi64, new_int_size, new_string_size);
  }
...
void array<T>::mutate_to_size(int64_t int_size) {
  ...
  if (unlikely(int_size > array_inner::MAX_HASHTABLE_SIZE)) {
    php_critical_errorf ("max array size exceeded: int_size = %" PRIi64, int_size);
  }
